...
it('it will log the replication of a model with softdeletes', function () {
    ...
    $activityItems = History::all();

    $this->assertCount(2, $activityItems);

    $this->assertTrue($activityItems->every(fn (History $item): bool => $item->event === 'created' &&
...
it('can fetch soft deleted models', function () {
    app()['config']->set('history-tracking.subject_returns_soft_deleted_models', true);
...
it('can log activity to log named in the model', function () {
    ...
    $this->assertSame('custom_log', History::latest()->first()->log_name);
});
